...
foreach ($categoryImages as $category => $images) {
    ...
    foreach ($images as $filename => $url) {
        ...
        if (file_exists($filePath)) {
            echo "File already exists: {$category}/{$filename}\n";
            continue;
        }
        ...
        if ($imageContent !== false) {
            ...
            echo "Failed to download: {$category}/{$filename}\n";
        }
    }
}
...
foreach ($products as $product) {
    ...
    if (file_exists($fullPath)) {
        ...
        $product->save();
        echo "Updated product {$product->name} with image path: {$imagePath}\n";
    } else {
        echo "Warning: Image not found for product {$product->name}: {$imagePath}\n";
    }
}

echo "Image download and organization complete!\n"; 